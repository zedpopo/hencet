    ...
    try {
        ...
        case 'stiker':
            if (isMedia) {
                ...
                const url = args[0]
                if (!url.match(isUrl)) client.reply(from, 'Mpp, link yang Lo kirim tidak valid hyyung.', id)
                await client.sendStickerfromUrl(from, url)
                    .then((r) => {
                        if (!r && r !== undefined) client.sendText(from, 'Maaf, link yang Lo kirim tidak memuat gambar.')
                    })
            } else {
                client.reply(from, 'Gaada Gambar! Untuk membuka daftar perintah kirim #menu', id)
            }
            break
        case 'gif':
        case 'stikergif':
            ...
            if (args.length == 1){
                ...
                if(isGiphy){
                    ...
                    if(getGiphyCode){
                        ...
                        client.reply(from, " Yah Gagal membuat sticker gif", id)
                    }
                    ...
                    client.reply(from, "Saat ini sticker gif hanya bisa menggunakan link giphy saja hyung.", id)
                }
            }
            break
        case 'tiktok': {
            if (args.length !== 1) return client.reply(from, 'Maaf, link yang lo kirim tidak valid, mungkin muka lo kurang pantas', id)
            const url = args[0]
            if (!url.match(isUrl) && !url.includes('tiktok.com')) return client.reply(from, 'Maaf, link yang lo kirim tidak valid', id)
            await client.sendText(from, '*Scraping Metadata...*')
            await tiktok(url)
                .then((videoMeta) => {
                    const filename = videoMeta.authorMeta.name + '.mp4'
                    const caps = `*Metadata:*\nUsername: ${videoMeta.authorMeta.name} \nMusic: ${videoMeta.musicMeta.musicName} \nView: ${videoMeta.playCount.toLocaleString()} \nLike: ${videoMeta.diggCount.toLocaleString()} \nComment: ${videoMeta.commentCount.toLocaleString()} \nShare: ${videoMeta.shareCount.toLocaleString()} \nCaption: ${videoMeta.text.trim() ? videoMeta.text : '-'} \n\nDonasi: Lo bisa membantu popo membeli amer dengan me melalui https://saweria.co/zedpopo atau mentrakteer melalui https://trakteer.id/zedpopo \nTerimakasih.`
